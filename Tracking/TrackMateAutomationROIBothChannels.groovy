...
#@ Double (label = "Quality threshold", value = 1, stepSize = 0.1) threshold
#@ Integer (label = "Max frame gap", value = 1) frameGap
#@ Double (label = "Linking max distance", value = 0.3, stepSize = 0.1) linkingMax
#@ Double (label = "Gap-closing max distance", value = 0.6, stepSize = 0.1) closingMax
...
def main() {
	inputFolder.eachFileRecurse {
		...
		if (name.endsWith(".tif")) {
			process(it, inputFolder, outputFolder, 1)
			process(it, inputFolder, outputFolder, 2)
		}
	}
}

def process(file, src, dst, ch) {
	...
	settings.detectorSettings['TARGET_CHANNEL'] = ch
	...
	// if ch is 1 then use "_green.xml" in place of ".xml"
	// if ch is 2 then use "_red.xml" in place of ".xml"

	if (ch == 1) {
		filename = filename.replace(".xml", "_green.xml")
	} else if (ch == 2) {
		filename = filename.replace(".xml", "_red.xml")
	}
